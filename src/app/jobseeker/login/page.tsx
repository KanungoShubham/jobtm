...
import { HiPhone, HiLockClosed, HiEye, HiEyeOff, HiExclamationCircle, HiShieldCheck, HiLightningBolt, HiBadgeCheck, HiArrowLeft } from 'react-icons/hi';
...
export default function JobseekerLoginPage() {
  ...
  useEffect(() => {
    if (jobseekerAuth.getToken()) router.replace('/jobseeker/home');
  }, [router]);

  const digits = mobile.replace(/\D/g, '');

  const finish = (res: { token: string; userId: string; role: string; name?: string }) => {
    if (res.role !== 'jobseeker') { setError('This account does not have job seeker access.'); return; }
    jobseekerAuth.saveSession(res.token, { userId: res.userId, name: res.name, role: res.role });
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden bg-slate-50">
      <div className="relative z-10 w-full max-w-4xl grid lg:grid-cols-2 rounded-[2rem] overflow-hidden shadow-warm-lg bg-white border border-slate-100">
        ...
        <div className="hidden lg:flex flex-col justify-between p-10 text-white relative overflow-hidden" style={{ backgroundColor: COLOR }}>
          <div className="relative z-10">
            ...
            <h2 className="text-3xl font-bold leading-tight mb-4">Find your next<br />opportunity.</h2>
            <p className="text-white/75 text-sm max-w-xs">Verified companies, trusted hiring, and gig work that fits your life.</p>
          </div>
        ...
        <div className="p-8 sm:p-10 bg-white">
          ...
          <div className="flex mb-5 rounded-xl overflow-hidden border border-slate-200 bg-slate-50 p-1">
            ...
            <Link href="/jobseeker/register" className="flex-1 py-2 rounded-lg text-sm font-bold text-center text-slate-500 hover:text-slate-700 transition-colors">
              Sign Up
            </Link>
          </div>
          ...
          <p className="text-center lg:text-left text-slate-400 text-sm mt-6">
            New here?{' '}
            <Link href="/jobseeker/register" className="font-bold" style={{ color: COLOR }}>Create an account</Link>
          </p>
        </div>
      </div>
    </div>
