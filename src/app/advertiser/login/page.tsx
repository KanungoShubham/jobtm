...
import { HiPhone, HiLockClosed, HiEye, HiEyeOff, HiExclamationCircle, HiSpeakerphone, HiShieldCheck, HiLightningBolt, HiUserGroup, HiArrowLeft } from 'react-icons/hi';
...
export default function AdvertiserLoginPage() {
  ...
  useEffect(() => {
    if (advertiserAuth.getToken()) router.replace('/advertiser');
  }, [router]);

  const digits = mobile.replace(/\D/g, '');

  const finish = (res: { token: string; userId: string; role: string; name?: string }) => {
    if (res.role !== 'advertiser') { setError('This account does not have advertiser access.'); return; }
    advertiserAuth.saveSession(res.token, { userId: res.userId, name: res.name, role: res.role });
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden bg-gradient-to-br from-amber-50 via-white to-amber-100">
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute -bottom-1/4 -left-1/4 w-[450px] h-[450px] rounded-full blur-3xl bg-amber-200/30" />
      </div>
      ...
      <div className="relative z-10 w-full max-w-4xl grid lg:grid-cols-2 rounded-[2rem] overflow-hidden shadow-warm-lg bg-white/70 backdrop-blur-sm border border-white animate-fade-in-scale">
        <div className="hidden lg:flex flex-col justify-between p-10 text-white relative overflow-hidden" style={{ background: `linear-gradient(150deg, ${COLOR}, #FBBF24 65%, #FDE68A)` }}>
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl font-bold leading-tight mb-4">Welcome back to<br />AD Center.</h2>
            <p className="text-white/75 text-sm max-w-xs">Manage your banner ads and track performance.</p>
          </div>
        ...
        <div className="p-8 sm:p-10 bg-white">
          ...
          <div className="text-center lg:text-left mb-6">
            <div className="lg:hidden inline-flex items-center justify-center w-14 h-14 rounded-2xl mb-3 shadow-warm"
              style={{ background: `linear-gradient(135deg, ${COLOR}, #FBBF24)` }}>
            ...
            <p className="text-slate-400 text-sm">Sign in to manage your ads</p>
          </div>
          ...
          <div className="flex mb-5 rounded-xl overflow-hidden border border-slate-200 bg-slate-50 p-1">
            ...
            <Link href="/advertiser/register" className="flex-1 py-2 rounded-lg text-sm font-bold text-center text-slate-500 hover:text-slate-700 transition-colors">
              Sign Up
            </Link>
          </div>
          ...
          <p className="text-center lg:text-left text-slate-400 text-sm mt-6">
            New advertiser?{' '}
            <Link href="/advertiser/register" className="font-bold" style={{ color: COLOR }}>Register your business</Link>
          </p>
        </div>
      </div>
    </div>
