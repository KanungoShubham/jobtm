...
function ActivityCard({ item, onApprove, onReject, actioning }: {
  item: any; onApprove: (id: string) => void; onReject: (id: string) => void; actioning: string | null;
}) {
    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <button onClick={() => setExpanded(!expanded)} className="w-full p-4 text-left space-y-2.5">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-500">
              {item.provider_name ?? '—'}{item.provider_mobile ? ` · ${item.provider_mobile}` : ''}
            </p>
          </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          <span className="bg-sky-50 text-sky-600 text-[11px] font-semibold px-2.5 py-1 rounded-full capitalize">{item.category}</span>
          ...
          {item.is_free ? (
            <span className="bg-emerald-50 text-emerald-600 text-[11px] font-semibold px-2.5 py-1 rounded-full">🆓 Free</span>
          ) : (
            <span className="bg-violet-50 text-violet-600 text-[11px] font-semibold px-2.5 py-1 rounded-full">₹{item.price}/session</span>
          )}
        ...
        <div className="px-4 pb-4 border-t border-slate-50 space-y-4 pt-3">
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { icon: HiCash,           label: 'Payout (85%)', value: item.is_free ? 'Free' : `₹${Math.round((item.price ?? 0) * 0.85)} per session` },
              { icon: HiCalendar,       label: 'Submitted',   value: item.created_at ? new Date(item.created_at).toLocaleDateString('en-IN') : '—' },
            ].map((row, i) => (
          ...
          {item.activity_image_url && (
            <div>
              ...
              <img src={item.activity_image_url} alt="Activity" className="w-full max-h-48 object-cover rounded-xl border border-slate-100" />
            </div>
          )}

          <div>
            <button onClick={() => setAadharOpen(!aadharOpen)}
            ...
            {aadharOpen && (
              item.aadhar_image_url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={item.aadhar_image_url} alt="Aadhar Card" className="w-full max-h-52 object-contain rounded-xl border-2 border-violet-200 bg-slate-50" />
              ) : (
      ...
      {item.status === 'pending' && (
        <div className="flex gap-2 px-4 pb-4">
          <button onClick={() => onReject(item.id)} disabled={isBusy}
          ...
          <button onClick={() => onApprove(item.id)} disabled={isBusy}
...
function PaymentRow({ booking, onView }: { booking: any; onView: (b: any) => void }) {
  ...
  return (
    <button onClick={() => onView(booking)}
      ...
      <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${isHeld ? 'bg-amber-50' : 'bg-emerald-50'}`}>
        {isHeld ? <HiLockClosed className="w-4 h-4 text-amber-500" /> : <HiCheckCircle className="w-4 h-4 text-emerald-500" />}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-bold text-slate-900 truncate">{booking.booked_by_name ?? 'Student'}</p>
        <p className="text-[11px] text-slate-500 truncate">
          {booking.booked_by_mobile ?? '—'}{sessDate ? ` · ${sessDate.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}` : ''}
        </p>
      </div>
      <div className="text-right flex-shrink-0">
        <p className="text-sm font-extrabold text-slate-900">₹{booking.amount_paid ?? 0}</p>
        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${isHeld ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
...
function PaymentDetailModal({ booking, onClose, onRelease, actioning }: {
  booking: any; onClose: () => void; onRelease: (id: string) => void; actioning: string | null;
}) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-slate-50 rounded-xl p-3 space-y-1.5">
            <div className="flex justify-between text-xs"><span className="text-slate-500">Total Collected</span><span className="font-bold text-slate-900">₹{booking.amount_paid ?? 0}</span></div>
            <div className="flex justify-between text-xs"><span className="text-slate-500">Platform Fee ({booking.platform_fee_pct ?? 15}%)</span><span className="font-semibold text-red-500">−₹{pct15}</span></div>
            <div className="h-px bg-slate-200" />
            <div className="flex justify-between text-xs"><span className="font-bold text-slate-700">Provider Payout (85%)</span><span className="font-extrabold text-emerald-600">₹{pct85}</span></div>
          </div>
          <div>
            ...
            {booking.payout_date && <Row label="Payout released" value={new Date(booking.payout_date).toLocaleDateString('en-IN')} />}
          </div>

          {canRelease ? (
            <button onClick={() => onRelease(booking.id)} disabled={isBusy}
              ...
              {isBusy ? <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <HiCash className="w-4 h-4" />}
...
export default function AdminActivitiesPage() {
  ...
  const hasMore = activities.length < actTotal;
  ...
  const loadActivitiesFirst = useCallback(async () => {
    if (tab === 'payments') return;
    setLoading(true); setError(''); setActivities([]); setActPage(1); setActTotal(0);
    try {
      const res = await activitiesAdminApi.list(token, { status: tab, page: 1, limit: PAGE_SIZE });
  ...
  const loadActivitiesNext = useCallback(async (nextPage: number) => {
    if (loadingMore) return;
    setLoadingMore(true);
    try {
      const res = await activitiesAdminApi.list(token, { status: tab, page: nextPage, limit: PAGE_SIZE });
  ...
  useEffect(() => {
    ...
    else loadActivitiesFirst();
  }, [tab, loadActivitiesFirst, loadPayments]);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMore && !loading && !loadingMore) loadActivitiesNext(actPage + 1);
      },
      { rootMargin: '120px' },
    );
    observer.observe(sentinel);
    return () => observer.disconnect();
  }, [tab, hasMore, loading, loadingMore, loadActivitiesNext, actPage]);
  ...
  const handleRelease = async (bookingId: string) => {
    ...
    try {
      ...
      setPayments((prev) => prev.map((p) => p.id === bookingId ? { ...p, ...patch } : p));
  ...
  const paymentGroups = (() => {
    ...
    for (const p of filteredPayments) {
      ...
      if (!map.has(key)) map.set(key, { activity_id: key, title: p.activity_title ?? 'Activity', category: p.activity_category ?? 'other', provider: p.provider_name ?? '—', rows: [], total: 0 });
      const g = map.get(key)!;
      g.rows.push(p);
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
            <HiAcademicCap className="w-6 h-6 text-sky-500" /> Extra Curricular Activities
          </h1>
          <p className="text-sm text-slate-400 mt-0.5">Verify providers · approve Aadhar · manage session payouts</p>
        </div>
        <div className="flex items-center gap-3">
          {tab !== 'payments' && actTotal > 0 && (
          ...
          <button onClick={() => { if (tab === 'payments') loadPayments(); else loadActivitiesFirst(); }} disabled={loading}
      ...
      {rejectTarget && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">Reason / Note for Provider (optional)</label>
              <textarea value={rejectNote} onChange={(e) => setRejectNote(e.target.value)}
                placeholder="e.g. Aadhar image unclear, please resubmit with a sharper photo…" rows={3}
      ...
      {loading ? (
            ...
            {paymentGroups.map((g) => {
              ...
              return (
                <div key={g.activity_id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                  <button onClick={() => toggleCollapse(g.activity_id)}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-slate-500">{g.provider} · {g.rows.length} booking{g.rows.length > 1 ? 's' : ''}</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                    ...
                    {isCollapsed ? <HiChevronDown className="w-5 h-5 text-slate-400 flex-shrink-0" /> : <HiChevronUp className="w-5 h-5 text-slate-400 flex-shrink-0" />}
                  </button>
                  {!isCollapsed && (
                    <div className="p-2.5 space-y-2">
                      {g.rows.map((p) => <PaymentRow key={p.id} booking={p} onView={setDetailBooking} />)}
                    </div>
                  )}
                </div>
              );
        ...
        <div className="flex flex-col items-center py-16 gap-3">
          ...
          <p className="text-base font-semibold text-slate-400">No {tab} activities</p>
        </div>
      ) : (
        <>
          <div className="space-y-3">
            {filteredActivities.map((item) => (
              <ActivityCard key={item.id} item={item} onApprove={handleApprove} onReject={(id) => setRejectTarget(id)} actioning={actioning} />
            ))}
          </div>
          ...
          <div ref={sentinelRef} className="py-4 flex justify-center">
            {loadingMore ? (
              ...
              <p className="text-xs text-slate-300 font-medium">All {actTotal} activities loaded</p>
            ) : null}
          </div>
        </>
      )}
      ...
      {detailBooking && (
        <PaymentDetailModal
          booking={detailBooking} onClose={() => setDetailBooking(null)}
          onRelease={handleRelease} actioning={actioning}
        />
      )}
    </div>
  );
