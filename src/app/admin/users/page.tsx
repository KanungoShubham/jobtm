...
function InfoRow({ icon: Icon, label, value, href }: { icon?: any; label: string; value: string; href?: string }) {
  return (
    <div className="flex items-center gap-3 px-4 py-3 border-t border-slate-50 first:border-0">
      ...
      {href ? (
        <a href={href} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 font-medium hover:underline truncate">{value}</a>
      ) : (
...
function UserDetailPanel({ userId, onClose, onUpdate }: { userId: string | null; onClose: () => void; onUpdate: () => void }) {
  ...
  const action = async (updates: object) => {
    ...
    try { await usersApi.updateStatus(getAdminToken() ?? '', data.id, updates); onUpdate(); onClose(); }
          ...
          <div className="flex-1 p-5 space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <div key={doc.label} className={`flex items-center gap-2 rounded-xl px-3 py-3 border ${doc.url ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-slate-200'}`}>
                        <doc.icon className={`w-5 h-5 ${doc.url ? 'text-blue-500' : 'text-slate-400'}`} />
                        <div className="flex-1 min-w-0">
                          <p className="text-[10px] text-slate-400">{doc.label}</p>
                          {doc.url ? (
                            <a href={doc.url} target="_blank" rel="noopener noreferrer" className="text-xs font-bold text-blue-600 hover:underline flex items-center gap-1">
            ...
            {!isEmployer && skills.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {skills.map((s) => (
                    <span key={s} className="bg-slate-100 text-slate-600 text-xs font-medium px-3 py-1.5 rounded-full">{s}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {showRejectInput && (
              <div className="space-y-2">
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider">Rejection Reason (optional)</label>
                <textarea
                  ...
                  rows={3} placeholder="Reason for rejection…" value={rejectNote}
                  onChange={(e) => setRejectNote(e.target.value)}
                />
                <div className="flex gap-2">
                  <button onClick={() => setShowRejectInput(false)} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-sm font-semibold hover:bg-slate-50 transition">Cancel</button>
                  <button onClick={() => action({ verify_status: 'rejected', rejection_note: rejectNote || undefined })} disabled={updating}
            ...
            {!showRejectInput && (
              <div className="space-y-2.5 sticky bottom-0 bg-white pt-3 pb-1 border-t border-slate-100 -mx-5 px-5">
                {status === 'pending' && (
                  <button disabled={updating} onClick={() => action({ verify_status: 'approved' })}
                ...
                <div className="flex gap-2.5">
                  <button disabled={updating} onClick={() => action({ is_active: !isActive })}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-2xl text-sm font-bold border transition ${isActive ? 'bg-red-50 border-red-200 text-red-500 hover:bg-red-100' : 'bg-emerald-50 border-emerald-200 text-emerald-600 hover:bg-emerald-100'}`}>
                    <HiBan className="w-4 h-4" />
                    {isActive ? 'Suspend' : 'Activate'}
                  </button>
                  {status === 'pending' && (
                    <button disabled={updating} onClick={() => setShowRejectInput(true)}
...
export default function AdminUsersPage() {
  ...
  const loadFirst = useCallback(async () => {
    setLoading(true); setError(''); setUsers([]); setPage(1); setTotal(0);
    try {
      const res = await usersApi.list(getAdminToken() ?? '', buildParams(1));
      setUsers(res.data ?? []);
  ...
  const loadNext = useCallback(async (nextPage: number) => {
    ...
    setLoadingMore(true);
    try {
      const res = await usersApi.list(getAdminToken() ?? '', buildParams(nextPage));
        ...
        <>
              ...
              return (
                <div key={u.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-3">
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${STATUS_PILL[status] ?? 'bg-slate-100 text-slate-500'}`}>{status}</span>
          ...
          <div ref={sentinelRef} className="py-4 flex justify-center">
            {loadingMore ? (
              ...
              <p className="text-xs text-slate-300 font-medium">All {total} users loaded</p>
            ) : null}
          </div>
        </>
      )}

      <UserDetailPanel userId={selectedId} onClose={() => setSelectedId(null)} onUpdate={loadFirst} />
    </div>
  );
}
